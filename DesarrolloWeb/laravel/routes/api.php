...
Route::prefix('/perro')->group(function () use ($router){
    ...
    $router->post('/Url', [PerroController::class, 'urlPerro']);
});
...
Route::prefix('/interaccion')->group(function () use ($router){
    ...
    $router->post('/guardarAceptado', [InteraccionController::class, 'guardarAceptado']);
    $router->post('/guardarRechazados', [InteraccionController::class, 'guardarRechazados']);
});
